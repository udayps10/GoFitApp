...
@WebServlet({"/GOFIT", "/GoFit"})
public class GoFitServlet extends HttpServlet {

    // Gemini API key is supplied through the GEMINI_API_KEY environment variable.
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse res)
        ...
        if (action.equals("register")) {
            ...
            if (user != null) {
                ...
                String redirectUrl = req.getContextPath() + "/GoFit?page=dashboard";
            ...
            try {
                ...
                if (ok) {
                    ...
                    int newCalorieGoal = fetchCalorieGoalFromAI(
                        user.getAge(), user.getWeightKg(), user.getHeightCm(), user.getGoal()
                    );
                    user.setCalorieGoal(newCalorieGoal);
                    userDAO.updateCalorieGoal(user.getId(), newCalorieGoal);
        ...
        } else if (action.equals("updateGoal")) {
            if (!isLoggedIn(session)) { res.sendRedirect(req.getContextPath() + "/login.jsp"); return; }
            User user = (User) session.getAttribute("user");

            try {
                String newGoal = req.getParameter("goal");
                if (newGoal == null || newGoal.trim().isEmpty()) {
                    res.sendError(400, "Missing goal value");
                    return;
                }

                int newCalorieGoal = fetchCalorieGoalFromAI(
                    user.getAge(), user.getWeightKg(), user.getHeightCm(), newGoal
                );

                boolean ok = userDAO.updateGoal(user.getId(), newGoal, newCalorieGoal);
                if (ok) {
                    ...
                    res.setContentType("application/json");
                    res.getWriter().write(
                        "{\"success\":true,\"goal\":\"" + newGoal + "\",\"calorieGoal\":" + newCalorieGoal + "}"
                    );
                } else {
                    res.sendError(500, "Goal update failed");
                }
            } catch (Exception e) {
                e.printStackTrace();
                res.sendError(500, "Invalid goal update: " + e.getMessage());
            }
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        System.out.println("📥 doGet called - page=" + page + ", sessionExists=" + (session != null));

        if (page == null) {
            System.out.println("⚠️ No page parameter, redirecting to homepage");
            res.sendRedirect(req.getContextPath() + "/homepage.jsp");
            return;
        }
        ...
        System.out.println("✅ User is logged in, processing page: " + page);
